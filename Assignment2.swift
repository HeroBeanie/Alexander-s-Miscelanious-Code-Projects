...
indirect enum MyList<A>{
    case cons(_ head: A,_ tail: MyList<A>)
    case empty
...
func foldLeft<B>(accum: B , fold: (B, A) -> B) -> B{
    switch self{
    case let .cons(head,tail):
    let temp = fold(accum,head)
    return tail.foldLeft(accum:temp, fold:fold)
    case .empty:
    return accum
    } 
}
...
func filter(predicate: (A) -> Bool) -> MyList<A>{
    switch self{
        case let .cons(head,tail):
        if predicate(head) {
            return MyList<A>.cons(head, tail.filter(predicate:predicate))
        }   
        else{
            return tail.filter(predicate:predicate)
        }
        case .empty:
        return MyList<A>.empty
    }
}
